  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        {/* Bottom fade — blends grid into surface color */}
        <div
        ...
        >
          <Pattern cellClassName="border-[var(--color-accent)] relative z-[100]" />
        </div>
        {/* Base grid — visible */}
        <Pattern className="opacity-100" cellClassName="border-black/[0.08]" />
      </div>
    </div>
  )
}
...
const Cell = ({ rowIdx, colIdx, clickedCell, setClickedCell, cellClassName }: CellProps) => {
  ...
  return (
    <div
      className={cn(
        "bg-transparent border-l border-b border-black/[0.08]",
        cellClassName
      )}
    ...
    >
      <motion.div
        ...
        animate={controls}
        className="h-12 w-12"
        style={{ backgroundColor: "rgba(247, 198, 59, 0.2)" }}
      />
    </div>
  )
}
